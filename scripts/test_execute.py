...
def executor(tmp_project, phase_dir):
    ...
    inst._settings_file = tmp_project / ".codex" / "settings.json"
    return inst
...
class TestInvokeCodex:
    ...
    def test_codex_bin_can_be_overridden(self, executor):
        executor._codex_bin = "/tmp/codex"
        cmd = executor._build_codex_command()
        assert cmd[0] == "/tmp/codex"

    def test_bypass_sandbox_uses_codex_danger_flag(self, executor):
        executor._bypass_sandbox = True
        cmd = executor._build_codex_command()
...
class TestValidationCommands:
    def test_loads_validation_commands_from_codex_settings(self, executor, tmp_project):
        settings_dir = tmp_project / ".codex"
        settings_dir.mkdir()
        settings_file = settings_dir / "settings.json"
        settings_file.write_text(json.dumps({
            "validation": {
                "commands": [
                    "./mvnw test -Dspring.jpa.properties.hibernate.hbm2ddl.auto=validate",
                    "",
                    123,
                ]
            }
        ...
        result = executor._load_validation_commands()

        assert result == ["./mvnw test -Dspring.jpa.properties.hibernate.hbm2ddl.auto=validate"]

    def test_no_settings_returns_empty_validation_commands(self, executor):
        assert executor._load_validation_commands() == []

    def test_run_validation_commands_success_saves_output(self, executor, tmp_project):
        settings_dir = tmp_project / ".codex"
        settings_dir.mkdir()
        ...
        result = executor._run_validation_commands(2)

        assert result is None
        output_file = executor._phase_dir / "step2-validation.json"
        data = json.loads(output_file.read_text())
        assert data["results"][0]["command"] == "echo ok"
        assert data["results"][0]["exitCode"] == 0

    def test_run_validation_commands_failure_returns_error(self, executor, tmp_project):
        settings_dir = tmp_project / ".codex"
        settings_dir.mkdir()
        (settings_dir / "settings.json").write_text(json.dumps({
            "validation": {"commands": ["sh -c 'echo schema mismatch; exit 7'"]}
        }))

        result = executor._run_validation_commands(2)

        assert "Validation command failed" in result
        assert "schema mismatch" in result
        output_file = executor._phase_dir / "step2-validation.json"
        data = json.loads(output_file.read_text())
        assert data["results"][0]["exitCode"] == 7
